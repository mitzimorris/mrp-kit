...
This vignette presents the typical workflow of `mrpkit`, an R package for implementing a "grammar" for multilevel regression with post-stratification.
...
`SurveyData` are objects used to represent both the survey and the post-stratification datasets, along with their metadata. The survey metadata consists of the survey's variables, the questions that were asked, levels of the responses, survey weights , and the survey design formula (random sample, one stage cluster sample, and stratified sample). Note that survey weights and survey design formula are optional to be included. 

The most important methods in `SurveyData` are `SurveyData$new` and `SurveyData$print`. The `SurveyData$new` method is used to transform a regular dataframe to a `SurveyData` object, whereas `SurveyData$print` is used to print the created `SurveyData` object. 
...
2. `questions`: The question asked in the questionnaire for each of those columns and its corresponding variable name.
...
1. `new`
    ...
    This method is used to initialize a new `SurveyMap` object. This takes `SurveyData` and `SurveyQuestion` objects as its arguments. 
...
### Get the estimated value of each post-stratification cell
...
### Aggregate the estimated value to a higher level of estimate
...
Once we have the aggregated estimates, we are able to visualize it using the `visify` method. This method takes the aggregated data frame as the argument. It generates a violin plot of the aggregated estimate by level of certain variable, in this case `age` and `state` and a density plot for the population estimates. In the example below, we show the example of the estimate of `age` and `state` variables. 
...
The plot above shows the distribution of NLP support for each age-group. The plot implies that people who are aged 56-65 are most likely to vote for the NLP.

The next plot displays the probability of voting for the NLP by state. We learn that the probability of the NLP winning the election is high since the majority of people (around 75%) in all of the states in Neverland vote for that party. Note that this violin plot is generated using `ggplot2` (Wickham, 2016). Hence, it could be modified the same way `ggplot2`'s plot modified. For example, here, we can add title and change the theme of the plot. 


```{r state-vis}
library(ggplot2)
plot_state <- fit1$visify(state_preds) + 
  ggtitle("Probability of voting the NLP by state") +
  theme_bw()
plot_state
```
